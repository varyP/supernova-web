...
const MintToken: FC = () => {
	...
	const ERC_721A_ABI = [
		...
		"function FREE_MAX_SUPPLY() view returns (uint256)",
		"function MAX_PER_TX_FREE() view returns (uint256)",
		"function paused() view returns (bool)"
	]
	const [nftReadOnlyContract, setNftReadOnlyContract] = useState<Contract | null>(null);
	...
	const executeContractMethod = async(param: string, methodName: string, transactionOptions:object) => {
		try {
			...
			console.log("Calling ", methodName, "with Params", param, transactionOptions);
			switch(methodName) { 
				case "whitelistMint": { 
					console.log("Executing ", methodName);
					let gasEstimate = await nftContract[methodName](param);
					const tx = await nftContract[methodName](param, transactionOptions);
				}  
				default: { 
					console.log("Executing ", methodName);
					let freeSupplyMax = await nftReadOnlyContract.FREE_MAX_SUPPLY();
					let totalSupply = await  nftReadOnlyContract.totalSupply();
					let maxFreePerTxn = await  nftReadOnlyContract.MAX_PER_TX_FREE();
					let paused = await  nftReadOnlyContract.paused();
					let freeSupplyMaxInt = parseInt(freeSupplyMax.toString());
					let totalSupplyInt = parseInt(totalSupply.toString());
					let maxFreePerTxnInt = parseInt(maxFreePerTxn.toString());
					console.log("FREE SUPPLY : ", freeSupplyMax.toString()," Total supply: ", totalSupply.toString());
					console.log("PAUSED: ", paused);
					if (paused) {
						alert("Sale has not started yet!");
						return;
					}
					if (totalSupplyInt + parseInt(param) < freeSupplyMaxInt  && parseInt(param) <= maxFreePerTxnInt) {
						console.log("going for free supply")
						let gasEstimate = await nftContract[methodName](String(param));
					} else {
						const tx = await nftContract[methodName](String(param), transactionOptions);
					}
				} 
			 }
		} catch(error){
	...
	const handleOnConnect = (accounts: Array<string>) => {
		...
		if (accounts.length != 0 || connected) {
			...
			setNftReadOnlyContract(new ethers.Contract(CONTRACT_ADDRESS, ERC_721A_ABI, provider));
		} else {
			console.log("No Contract")
			setNFTContract(null);
		}
		...
		<section className="w-full bg-white" id="mint">
			{
				nftContract && connected ? (
					<div className="max-w-7xl mx-auto py-16 px-8 text-black">
						<h1 className="text-3xl font-extralight">Mint a 🦉</h1>
...
/*
["0x3d70a75f756c6e5d2780d0436ff8581b5c366fe9a26df44bfff926538a6b26b6","0x7e48b667a54ae9e0333b4bfb5814087fb3ed18bf86d2e7285b7cbab827d97e0b","0x0aa42e98b80a61ebc0d95c6460ff0e57b35fa7d0903b6b1fdf0dd46c6d5f623e","0xb056c39af7283c4b4395be3d4722d4bba3e4df3d97f912211abdd76b8c9a695d","0x65e5727500f6003f267ff7be3ea47524de5befe4de0d2263b6a523da73935e0c","0x129ad14d5e077fd1af32bf250334da5ab206760a986648bb8d4c2dab23e5e063"]
*/

/*
totalSupply = 0
free-max = 5
total + current < free-max
